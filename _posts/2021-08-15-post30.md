...
스레드의 기본 구현 메커니즘은 마하(Mach, 커널) 스레드이지만 마하 수준에서 스레드로 작업하는 경우는 거의 없습니다. 대신, 일반적으로 더 편리한 POSIX API 또는 파생상품 중 하나를 사용한다. 마하 구현은 모든 스레드의 기본 기능을 제공하지만, 여기에는 선점형 실행 모델과 스레드가 서로 독립적이도록 스케줄링하는 기능이 포함됩니다. 
